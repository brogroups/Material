<template>
    <div class="unique-salom">
      
      <label class="unique-label" @click="toggleCalendar">
        <img src="../assets/calendar.png" alt="" />
        <span v-if="selectedDate">{{ selectedDateString }}</span>
        <span v-else>Added days</span>
      </label>
      <aside ref="aside"></aside>
      <div class="unique-date-picker" v-if="showCalendar">
        ...
        <div class="unique-header">
          ...
          <div class="unique-dropdown" @click="showYears">
            {{ currentYear }}
          </div>
          <div class="unique-dropdown" v-if="!showYearDropdown && !showMonthDropdown" @click="showMonths">
        ...
        <div class="unique-body">
          <div v-if="showYearDropdown" class="unique-years">
            <div
              v-for="year in yearRange"
              :key="year"
              :class="['unique-year', { 'unique-selected': year === currentYear }]"
              @click="selectYear(year)"
            >
              {{ year }}
            </div>
          </div>
          <div v-else-if="showMonthDropdown" class="unique-months">
            <div
              v-for="(month, index) in monthNames"
              :key="month"
              :class="['unique-month', { 'unique-selected': index === currentMonth }]"
          ...
          <div v-else>
            <div class="unique-weekdays">
              <div v-for="day in weekdays" :key="day" class="unique-weekday">
                {{ day }}
              </div>
            </div>
            <div class="unique-days">
              <div
                ...
                :class="[
                  'unique-day',
                  {
                    'unique-today': date.isToday,
                    'unique-selected': date.isSelected,
                    'unique-not-current': date.notCurrent,
                  },
                ]"
